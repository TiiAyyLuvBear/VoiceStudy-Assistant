"""Merge the validation-selected ASR v4 LoRA adapter into Whisper Small."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--adapter", type=Path, default=Path("models/experimental/asr/v4/best_adapter")
    )
    parser.add_argument(
        "--training-summary",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("models/experimental/asr/v4/hf_merged")
    )
    ...
    if training.get("dataset_version") != "asr-v4":
        raise ValueError("Training summary is not ASR v4")
    if training.get("test_split_accessed") is not False:
        raise ValueError("Training summary indicates test access")

    processor = WhisperProcessor.from_pretrained(
        args.adapter, local_files_only=True, language="vi", task="transcribe"
    )
    base = WhisperForConditionalGeneration.from_pretrained(
        args.base_model, local_files_only=True, low_cpu_mem_usage=True
    )
    ...
    summary = {
        ...
        "best_validation": training["selection"]["best_validation"],
